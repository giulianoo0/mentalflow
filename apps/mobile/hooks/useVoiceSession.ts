...
export const DEFAULT_VOICE_ID: VoiceOptionId = "giuliano";

export const VOICE_OPTIONS: VoiceOption[] = [
  {
    id: "marcos",
    label: "Marcos",
    description: "Profundo e seguro",
    openAiVoice: "ash",
  },
  {
    id: "leticia",
    label: "Leticia",
    description: "Leve e gentil",
    openAiVoice: "coral",
  },
  {
    id: "giuliano",
    label: "Giuliano",
    description: "Firme e vibrante",
    openAiVoice: "cedar",
  },
  {
    id: "nataly",
...
export function useVoiceSession(options: UseVoiceSessionOptions = {}) {
  ...
  const searchWidgetsToolAction = useAction(
    (api as any).chat.searchWidgetsTool,
  );
  const upsertWidgetsToolAction = useAction(
    (api as any).chat.upsertWidgetsTool,
  );
  const updateWidgetMutation = useMutation((api as any).widgets.updateWidget);
  const createRealtimeSessionTokenAction = useAction(
    (api as any).voice.createRealtimeSessionToken,
  );
    ...
    async (event: any) => {
      switch (event.type) {
            ...
            if (flowNanoIdRef.current) {
              if (pendingUserMessageIdRef.current) {
                ...
                updateMessageContent({
                  messageId: pendingUserMessageIdRef.current as any,
                  content: userText,
                  isComplete: true,
                }).catch(console.error);
                ...
                insertMessage({
                  flowNanoId: flowNanoIdRef.current as any,
                  role: "user",
                  content: userText,
                  isComplete: true,
                }).catch(console.error);
        ...
        case "response.audio_transcript.done":
          const assistantText =
            event.transcript?.trim() || currentTranscript.trim();
          if (assistantText) {
            const t: Transcription = {
              id: event.item_id || `a-${Date.now()}`,
              role: "assistant",
              text: assistantText,
              isFinal: true,
            };
  ...
  const searchWidgets = useCallback(
    async (input: WidgetToolSearchArgs) => {
  ...
  const upsertWidgets = useCallback(
    async (input: WidgetToolUpsertArgs) => {
      const flowNanoIdValue = flowNanoIdRef.current;
      if (!flowNanoIdValue) throw new Error("Missing flowNanoId");
      return await upsertWidgetsToolAction({
        flowNanoId: flowNanoIdValue,
  ...
  const startSession = useCallback(async () => {
    ...
    try {
              ...
              tools: [
                {
                  ...
                  description:
                    "Buscar widgets existentes por título ou tipo para evitar duplicatas.",
                  parameters: {
                    type: "object",
                    properties: {
                ...
                },
                {
                  ...
                  description:
                    "Criar ou atualizar widgets e links com deduplicação por fingerprint.",
                  parameters: {
                    type: "object",
                    properties: {
                      widgets: {
                        ...
                        items: {
                          ...
                          properties: {
                            ...
                            dueDate: {
                              ...
                              nullable: true,
                              description:
                                "ISO 8601 com offset (ex: 2026-01-22T18:00:00-03:00)",
                            },
                            priority: {
                              type: "string",
      ...
      dc.onmessage = (e: any) => {
        if (isAbortedRef.current) return;
        try {
          void handleRealtimeEvent(JSON.parse(e.data));
        } catch (err) {}
      };
    ...
    createRealtimeSessionTokenAction,
    recorder,
    updateStatus,
    onError,
    cleanup,
